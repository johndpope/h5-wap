<template>
    <div>
                                        ...
                                        <div class="type-h-wrap">
                                            ...
                                            <div class="type-h-right">
                                                <p class="type-h-odd">
                                                    ...
                                                    <span class="onlywin" :class="{'click-hook': sport.Match_BzM > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_BzM,sport.Match_Master,'全场-独赢', 'Match_BzM',$event)">{{sport.Match_BzM > 0 ? sport.Match_BzM : ''}}</span>
                                                    <span class="pankou-wrap">
                          <span class="pankou" v-if="sport.Match_ShowType === 'H'">{{pankouFilter(sport.Match_RGG)}}</span>
                          <span class="h-ho" :class="{'click-hook': sport.Match_Ho > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_Ho,sport.Match_Master,'全场-让球', 'Match_Ho',$event)">{{sport.Match_Ho>0 ? sport.Match_Ho : ''}}</span>
                        </span>
                                                    <span class="dx-wrap">
                          <span class="dx-pkou">{{pankouFilter(sport.Match_DxGG1)}}</span>
                          <span class="dx-x" :class="{'click-hook': sport.Match_DxDpl > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_DxDpl,'大','全场-大小', 'Match_DxDpl',$event)">{{sport.Match_DxDpl > 0 ? sport.Match_DxDpl: ''}}</span>
                        </span>
                                                    <span class="ds-d-wrap" :class="{'click-hook': sport.Match_DsDpl > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_DsDpl,'单','全场-单双', 'Match_DsDpl',$event)">{{sport.Match_DsDpl > 0 ? '单' + sport.Match_DsDpl : ''}}</span>
                                                </p>
                                                <p class="type-h-odd">
                                                    <strong class="type-name">客</strong>
                                                    <span class="onlywin" :class="{'click-hook': sport.Match_BzG > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_BzG,sport.Match_Guest,'全场-独赢', 'Match_BzG',$event)">{{sport.Match_BzG>0?sport.Match_BzG:''}}</span>
                                                    <span class="pankou-wrap">
                          <span class="pankou" v-if="sport.Match_ShowType === 'C'">{{pankouFilter(sport.Match_RGG)}}</span>
                          <span class="h-ho" :class="{'click-hook': sport.Match_Ao > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_Ao,sport.Match_Guest,'全场-让球', 'Match_Ao',$event)">{{sport.Match_Ao>0?sport.Match_Ao:''}}</span>
                        </span>
                                                    <span class="dx-wrap">
                          <span class="dx-pkou">{{pankouFilter(sport.Match_DxGG2)}}</span>
                          <span class="dx-x" :class="{'click-hook': sport.Match_DxXpl > 0}" :data-id="sport.Match_ID"@click="submitOrder(sport,sport.Match_DxXpl,'小','全场-大小', 'Match_DxXpl',$event)">{{sport.Match_DxXpl>0?sport.Match_DxXpl:''}}</span>
                        </span>
                                                    <span class="ds-s-wrap" :class="{'click-hook': sport.Match_DsSpl > 0}":data-id="sport.Match_ID" @click="submitOrder(sport,sport.Match_DsSpl,'双','全场-单双', 'Match_DsSpl',$event)">{{sport.Match_DsSpl>0?'双'+sport.Match_DsSpl:''}}</span>
                                                </p>
                                            </div>
                                        </div>
                                    </div>
        ...
        <sport-submit :ifsubmitShow="ifsubmitShow"  :sportsArr="sportsArr"
                      ...
                      :cgodd="cgodd" :cgnum ="cgnum"
                      @toggleIfSubmit="toggleIfSubmit" :ifchuanguan = "false" @on-cgnum="cgnumVariety" @resetData="resetData"></sport-submit>
    ...
    export default {
        ...
        methods: {
            ...
            cgnumVariety(id){
                ...
                che.forEach(function (v,i) {
                    let index = i;
                    v.forEach(function (o,d) {
                        if(o.id==id){
                            che.splice(index,1)
                        }
                    })
            ...
            //    下注成功，清空选择
            resetData(){
//          let act   =  document.getElementsByClassName('info-content-wrap');
